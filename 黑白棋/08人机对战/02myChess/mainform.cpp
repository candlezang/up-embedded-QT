...
//    qDebug() << x << y;
    ...
    if(num > 0){
        if(currentPK == PVP || currentPK == PVC){
            //通知棋盘,改变棋盘显示数据
        ...
        if(currentPK == PVC){
            ...
            for(int i=0; i<8; i++){
                for(int j=0; j<8; j++){
                    if(chessData[i][j] == classChess::Empty){
                        num = judgeRule(i, j, chessData, currentRole, false);
                        if(num > max){
                            max = num;
                            x = i;
                            y = j;
            ...
            //真正落子
            judgeRule(x, y, chessData, currentRole, true);
...
//09.01 实现人机对战
void MainForm::on_btn_pvc_clicked()
{
    currentPK = PVC;
    chessInit();
    myChess->setChangeChessData(&chessData);
